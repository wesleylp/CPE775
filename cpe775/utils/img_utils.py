...
def show_landmarks_batch(sample_batched, normalized=True, nrow=8, padding=2):
    ...
    for j in range(ncol):
        for i in range(nrow):
            ...
            plt.scatter(
                landmarks_batch[i + j * nrow, :, 0].numpy() * im_size + i * im_size + padding * i,
...
def increase_img_size(img, bbox):
    ...
    if type(img) == Image.Image:
        expand_img = F.pad(img, (x_extra_start, y_extra_start, x_extra_end, y_extra_end))
        width, height = expand_img.size
    else:
        expand_img = cv2.copyMakeBorder(
            img,
            y_extra_start,
            y_extra_end,
            x_extra_start,
...
def rect_to_tuple(rect, padding=0):
    return (int(rect.left()) - padding, int(rect.top()) - padding, int(rect.right()) + padding,
